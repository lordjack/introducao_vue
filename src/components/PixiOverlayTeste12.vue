  ...
  <div class="hello">
    ...
    <div id="map" style="height: 100%; width: 100%" class="cartes">
      ...
      <div class="main-legend hide" style="bottom: 125px">
        <ul>
          <li>
            <span class="color" style="background-color: #b61d1d"></span>Extrême
            gauche
          </li>
          <li>
            <span class="color" style="background-color: #dc2a1b"></span>PC -
            LFI
          </li>
          <li>
            <span class="color" style="background-color: #f19ec7"></span>PS -
            RDG - DVG
          </li>
          <li>
            <span class="color" style="background-color: #23db23"></span
            >Écologiste
          </li>
          <li>
            <span class="color" style="background-color: #d5e4ed"></span>Divers
          </li>
          <li>
            <span class="color" style="background-color: #d7d7d7"></span
            >Régionaliste
          </li>
          <li>
            <span class="color" style="background-color: #fed002"></span>REM
          </li>
          <li>
            <span class="color" style="background-color: #fe9d00"></span>MODEM
          </li>
          <li>
            <span class="color" style="background-color: #78bce0"></span>UDI -
            LR - DVD
          </li>
          <li>
            <span class="color" style="background-color: #0089c4"></span>DLF
          </li>
          <li>
            <span class="color" style="background-color: #83726d"></span>FN
          </li>
          <li>
            <span class="color" style="background-color: #3e4142"></span>Extrême
            droite
          </li>
          <li>
            <span class="color" style="background-color: #ffffff"></span>Pas de
            résultats
          </li>
        </ul>
...
import 'leaflet.photon/leaflet.photon';
...
import graphDraw from 'graph-draw';
...
export default {
  ...
  methods: {
    ...
    async draw() {
      ...
      window.L.Control.Legend = window.L.Control.extend({
        onAdd: (map) => {
      ...
      window.L.control.legend = function(opts) {
        return new window.L.Control.Legend(opts);
      };

      window.L.control.legend({ position: 'bottomright' }).addTo(vm.map);
          ...
          for (i = 0, len = polygon.length; i < len; i++) {
            ...
            for (j = 0, len2 = part.length, k = len2 - 1; j < len2; k = j++) {
              ...
              if (
                p1[1] > p.y !== p2[1] > p.y &&
                p.x <
                  ((p2[0] - p1[0]) * (p.y - p1[1])) / (p2[1] - p1[1]) + p1[0]
              ) {
                inside = !inside;
              }
            }
                    ...
                    circs.coordinates.forEach(function(arc) {
                      arc.forEach(function(point, index) {
                        ...
                        if (index > 0) {
                          var prevPoint = arc[index - 1];
                          var indexFrom =
                            point2index2[prevPoint[0] + '#' + prevPoint[1]];
                ...
                function findFeature(latlng) {
                  ...
                  for (var i = 0; i < features.length; i++) {
                    ...
                    if (feat.geometry.type === 'Polygon') {
                      ...
                      for (
                        var j = 0;
                        j < feat.geometry.coordinates.length;
                        j++
                      ) {
                  ...
                  .photon({
                    ...
                    onSelected: function(feature) {
                      ...
                      features.every(function(item) {
                        ...
                        if (ref[0] === '9') {
                          insee =
                            ref.substring(0, 2) + item.feature.properties.city;
                        } else {
                          insee =
                            ref.substring(1, 3) + item.feature.properties.city;
                        }
                        if (insee === code) {
                          feat = item.feature;
                          return false;
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      if (this.map) {
        var searchControl = window.L.control.photon();
